...
function reasonLabel$1(reason, t) {
	switch (reason) {
		...
		children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
			...
			children: [
				...
				canUpdate ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
					className: PluginManageSettingsTab_module_css_default.iconButton,
					type: "button",
					title: item.packageName === "dsh-plugin-console" ? t("ariaUpdateManager") : t("ariaUpdate"),
					"aria-label": item.packageName === "dsh-plugin-console" ? t("ariaUpdateManager") : t("ariaUpdate"),
					onClick: onUpdate,
		...
		children: [
			...
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					canUpdate ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
						className: PluginManageSettingsTab_module_css_default.primaryButton,
						type: "button",
						disabled: working,
						onClick: onUpdate,
...
function PluginManageSettingsTab({ api, locale, t }) {
	...
	const loadBootstrap = () => {
		...
		api.bootstrap(request, language).then((value) => {
	...
	(0, react.useEffect)(() => {
		if (load.status !== "ready") return;
		let live = true;
		api.installed(language).then((value) => {
	...
	const applyOperationResult = (result, sequence) => {
		...
		if (result.status === "succeeded") {
			setBanner(t(result.canary === "passed" ? "canaryPassedBanner" : result.action === "remove" ? "removeRestartBanner" : "restartBanner"));
			setSelection(null);
		} else {
			const reason = result.detail ?? reasonLabel$1(result.code, t);
	...
	const startPlan = (requestToPlan, title, immediate = false) => {
		const sequence = ++operationSequence.current;
		setWorking(true);
		setOperationError(null);
		api.plan(requestToPlan).then((plan) => {
			if (!alive.current || sequence !== operationSequence.current) return;
			if (plan.status === "blocked") setOperationError(interpolate(t("blocked", { reason: "" }), { reason: reasonLabel$1(plan.blockReason, t) }));
			else if (immediate && plan.planId !== null) return api.execute(plan.planId).then((result) => {
				applyOperationResult(result, sequence);
			});
			else setReview({
				plan,
				title
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								className: PluginManageSettingsTab_module_css_default.iconButton,
								type: "button",
								disabled: working || load.status !== "ready",
								title: t("ariaRefresh"),
...
function Pagination({ page, pages, t, onPage, disabled }) {
...
//#endregion
//#region \0plugin-console-css:src/client/HarnessUpdateAction.module.css.mjs
const css = ".w1wjFW_trigger{color:var(--dsh-color-text-secondary,#9ca3af);cursor:pointer;background:0 0;border:0;border-radius:8px;align-items:center;gap:6px;padding:6px;font-size:12px;line-height:1;display:flex;position:relative}.w1wjFW_trigger:hover,.w1wjFW_trigger:focus-visible{background:var(--dsh-color-surface-hover,#7f7f7f1f);color:var(--dsh-color-text,#e5e7eb);outline:none}.w1wjFW_trigger:disabled{cursor:default;opacity:.6}.w1wjFW_triggerLabel{text-overflow:ellipsis;white-space:nowrap;overflow:hidden}.w1wjFW_badgeAvailable,.w1wjFW_badgePending{pointer-events:none;border:1px solid #0006;border-radius:50%;width:8px;height:8px;position:absolute;top:3px;right:3px}.w1wjFW_badgeAvailable{background:#f59e0b}.w1wjFW_badgePending{background:#34d399}.w1wjFW_spin{animation:1s linear infinite w1wjFW_pluginConsoleSpin}@keyframes w1wjFW_pluginConsoleSpin{0%{transform:rotate(0)}to{transform:rotate(360deg)}}.w1wjFW_panel{z-index:1000;border:1px solid var(--dsh-color-border,#7f7f7f40);background:var(--dsh-color-surface,#1f2430);color:var(--dsh-color-text,#e5e7eb);border-radius:12px;flex-direction:column;display:flex;position:fixed;overflow:hidden;box-shadow:0 12px 32px #00000073}.w1wjFW_panelHeader{border-bottom:1px solid var(--dsh-color-border,#7f7f7f40);justify-content:space-between;align-items:center;gap:8px;padding:10px 12px;display:flex}.w1wjFW_panelHeader h3{margin:0;font-size:14px;font-weight:600}.w1wjFW_iconButton{color:var(--dsh-color-text-secondary,#9ca3af);cursor:pointer;background:0 0;border:0;border-radius:6px;justify-content:center;align-items:center;padding:5px;display:inline-flex}.w1wjFW_iconButton:hover,.w1wjFW_iconButton:focus-visible{background:var(--dsh-color-surface-hover,#7f7f7f1f);color:var(--dsh-color-text,#e5e7eb);outline:none}.w1wjFW_panelBody{flex-direction:column;gap:10px;padding:12px;display:flex;overflow-y:auto}.w1wjFW_metaGrid{grid-template-columns:auto 1fr;gap:6px 12px;margin:0;font-size:12px;display:grid}.w1wjFW_metaGrid dt{color:var(--dsh-color-text-secondary,#9ca3af)}.w1wjFW_metaGrid dd{overflow-wrap:anywhere;min-width:0;margin:0}.w1wjFW_breakable{overflow-wrap:anywhere}.w1wjFW_lead{margin:0;font-size:13px;font-weight:600}.w1wjFW_status{color:var(--dsh-color-text-secondary,#9ca3af);margin:0;font-size:12px}.w1wjFW_pending{color:#34d399;background:#34d3991f;border-radius:8px;margin:0;padding:8px 10px;font-size:12px}.w1wjFW_banner{color:#34d399;background:#34d3991f;border-radius:8px;align-items:flex-start;gap:8px;margin:0;padding:8px 10px;font-size:12px;display:flex}.w1wjFW_errorBanner{color:#f87171;overflow-wrap:anywhere;background:#f871711f;border-radius:8px;margin:0;padding:8px 10px;font-size:12px}.w1wjFW_reviewWarning{color:#fbbf24;background:#f59e0b1f;border-radius:8px;align-items:flex-start;gap:8px;margin:0;padding:8px 10px;font-size:12px;display:flex}.w1wjFW_panelActions{justify-content:flex-end;align-items:center;gap:8px;display:flex}.w1wjFW_primaryButton,.w1wjFW_ghostButton,.w1wjFW_refreshButton{cursor:pointer;border:1px solid #0000;border-radius:8px;align-items:center;gap:6px;padding:7px 12px;font-size:12px;display:inline-flex}.w1wjFW_primaryButton{background:var(--dsh-color-accent,#4d7cfe);color:#fff}.w1wjFW_primaryButton:hover,.w1wjFW_primaryButton:focus-visible{filter:brightness(1.1);outline:none}.w1wjFW_ghostButton{border-color:var(--dsh-color-border,#7f7f7f40);color:var(--dsh-color-text,#e5e7eb);background:0 0}.w1wjFW_ghostButton:hover,.w1wjFW_ghostButton:focus-visible{background:var(--dsh-color-surface-hover,#7f7f7f1f);outline:none}.w1wjFW_refreshButton{color:var(--dsh-color-text-secondary,#9ca3af);background:0 0;align-self:flex-start;padding:2px 4px}.w1wjFW_refreshButton:hover,.w1wjFW_refreshButton:focus-visible{color:var(--dsh-color-text,#e5e7eb);outline:none}button:disabled{cursor:default;opacity:.55}";
const tagId = "dsh-plugin-console/HarnessUpdateAction.module.css";
...
var HarnessUpdateAction_module_css_default = {
	"badgeAvailable": "w1wjFW_badgeAvailable",
	"badgePending": "w1wjFW_badgePending",
	"banner": "w1wjFW_banner",
	"breakable": "w1wjFW_breakable",
	"errorBanner": "w1wjFW_errorBanner",
	"ghostButton": "w1wjFW_ghostButton",
	"iconButton": "w1wjFW_iconButton",
	"lead": "w1wjFW_lead",
	"metaGrid": "w1wjFW_metaGrid",
	"panel": "w1wjFW_panel",
	"panelActions": "w1wjFW_panelActions",
	"panelBody": "w1wjFW_panelBody",
	"panelHeader": "w1wjFW_panelHeader",
	"pending": "w1wjFW_pending",
	"pluginConsoleSpin": "w1wjFW_pluginConsoleSpin",
	"primaryButton": "w1wjFW_primaryButton",
	"refreshButton": "w1wjFW_refreshButton",
	"reviewWarning": "w1wjFW_reviewWarning",
	"spin": "w1wjFW_spin",
	"status": "w1wjFW_status",
	"trigger": "w1wjFW_trigger",
	"triggerLabel": "w1wjFW_triggerLabel"
};

//#endregion
//#region src/client/HarnessUpdateAction.tsx
...
function HarnessUpdateAction({ api, wide, t }) {
	...
	const [banner, setBanner] = (0, react.useState)(null);
	const [operationError, setOperationError] = (0, react.useState)(null);
	const [position$3, setPosition] = (0, react.useState)(null);
	const triggerRef = (0, react.useRef)(null);
	const panelRef = (0, react.useRef)(null);
	const alive = (0, react.useRef)(true);
	const load = (refresh = false) => {
		api.harnessStatus(refresh).then((value) => {
			if (alive.current) {
				setStatus(value);
				setLoadError(null);
			}
	...
	const toggle = () => {
		...
		setPosition({
			top: rect === void 0 ? 96 : Math.max(8, rect.top - PANEL_MAX_HEIGHT - 8),
			left: rect === void 0 ? 8 : Math.max(8, Math.min(rect.left, window.innerWidth - width - 8))
		});
		setBanner(null);
		setOperationError(null);
		setOpen(true);
	...
	const startPlan = () => {
		...
		setOperationError(null);
		setBanner(null);
		api.harnessPlan().then((plan) => {
			if (!alive.current) return;
			if (plan.status === "blocked") setOperationError(t("harnessUpdateFailed", { message: reasonLabel(plan.blockReason, t) }));
			else setReview(plan);
		}).catch((error) => {
			if (!alive.current) return;
	...
	const confirmPlan = () => {
		...
		if (planId === null || planId === void 0) return;
		setWorking(true);
		setOperationError(null);
		api.harnessExecute(planId).then((result) => {
			if (!alive.current) return;
			setReview(null);
			if (result.status === "succeeded") setBanner(t("harnessUpdated", {
				from: result.currentVersion ?? "?",
				to: result.targetVersion ?? "?"
			}));
			else setOperationError(t("harnessUpdateFailed", { message: `${result.detail ?? reasonLabel(result.code, t)}${result.rollback === "succeeded" ? ` ${t("harnessUpdatedRollbackNote", { version: result.currentVersion ?? "?" })}` : ""}` }));
			load();
		}).catch((error) => {
			if (!alive.current) return;
			setOperationError(t("harnessUpdateFailed", { message: error instanceof Error ? error.message : String(error) }));
	...
	const title = status === null ? t("harnessAria") : status.updateAvailable ? t("harnessUpdate", { version: status.latestVersion ?? "?" }) : `${t("harnessTitle")} ${status.currentVersion ?? "?"}`;
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
		ref: triggerRef,
		className: HarnessUpdateAction_module_css_default.trigger,
		type: "button",
		title,
		...
		onClick: toggle,
		children: [
			working ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(__deepseek_ai_dsh_client_ui_primitives.IconRefreshOutline16, {
				"aria-hidden": "true",
				className: HarnessUpdateAction_module_css_default.spin
			}) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)(__deepseek_ai_dsh_client_ui_primitives.IconRefreshOutline16, { "aria-hidden": "true" }),
			wide ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
				className: HarnessUpdateAction_module_css_default.triggerLabel,
				children: t("harnessAction")
			}) : null,
			badge !== "none" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
				className: badge === "available" ? HarnessUpdateAction_module_css_default.badgeAvailable : HarnessUpdateAction_module_css_default.badgePending,
				"aria-hidden": "true"
			}) : null
		]
	}), open && position$3 !== null ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		ref: panelRef,
		className: HarnessUpdateAction_module_css_default.panel,
		role: "dialog",
		"aria-label": t("harnessAria"),
		style: {
			top: position$3.top,
		...
		children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
			className: HarnessUpdateAction_module_css_default.panelHeader,
			children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h3", { children: t("harnessTitle") }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
				className: HarnessUpdateAction_module_css_default.iconButton,
				type: "button",
				disabled: working,
				title: t("harnessClose"),
				"aria-label": t("harnessClose"),
				onClick: close,
				children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(__deepseek_ai_dsh_client_ui_primitives.IconCloseOutline16, { "aria-hidden": "true" })
			})]
		}), review !== null ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
			className: HarnessUpdateAction_module_css_default.panelBody,
			children: [
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
					className: HarnessUpdateAction_module_css_default.lead,
					children: t("harnessReviewTitle")
				}),
				/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("dl", {
					className: HarnessUpdateAction_module_css_default.metaGrid,
					children: [
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("dt", { children: t("harnessRunning") }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("dd", { children: review.currentVersion ?? "?" })] }),
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("dt", { children: t("harnessReviewTarget") }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("dd", { children: review.targetVersion ?? "?" })] }),
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("dt", { children: t("harnessReviewCommand") }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("dd", { children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("code", {
							className: HarnessUpdateAction_module_css_default.breakable,
							children: review.updateCommand ?? "?"
						}) })] })
					]
				}),
				/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("p", {
					className: HarnessUpdateAction_module_css_default.reviewWarning,
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(__deepseek_ai_dsh_client_ui_primitives.IconWarningOutline16, { "aria-hidden": "true" }), t("harnessReviewWarning")]
				}),
				/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					className: HarnessUpdateAction_module_css_default.panelActions,
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
						className: HarnessUpdateAction_module_css_default.ghostButton,
						type: "button",
						disabled: working,
						onClick: () => setReview(null),
						children: t("harnessCancel")
					}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
						className: HarnessUpdateAction_module_css_default.primaryButton,
						type: "button",
						disabled: working,
						onClick: confirmPlan,
						children: working ? t("harnessUpdating") : t("harnessConfirm")
					})]
				})
			]
		}) : /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
			...
			children: [
				banner !== null ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					className: HarnessUpdateAction_module_css_default.banner,
					role: "status",
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(__deepseek_ai_dsh_client_ui_primitives.IconCheckOutline16, { "aria-hidden": "true" }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: banner })]
				}) : null,
				operationError !== null ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
					className: HarnessUpdateAction_module_css_default.errorBanner,
					role: "alert",
					children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: operationError })
				}) : null,
				loadError !== null ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
					className: HarnessUpdateAction_module_css_default.errorBanner,
					role: "alert",
					children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: t("harnessCheckFailed", { message: loadError }) })
				}) : status === null ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
					className: HarnessUpdateAction_module_css_default.status,
					children: t("harnessChecking")
				}) : status.managed ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("dl", {
					className: HarnessUpdateAction_module_css_default.metaGrid,
					children: [
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("dt", { children: t("harnessRunning") }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("dd", { children: status.currentVersion ?? "?" })] }),
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("dt", { children: t("harnessInstalled") }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("dd", { children: status.installedVersion ?? "?" })] }),
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("dt", { children: t("harnessLatest") }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("dd", { children: status.latestVersion ?? "?" })] }),
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("dt", { children: t("harnessTag") }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("dd", { children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("code", { children: status.updateTag ?? "-" }) })] })
					]
				}), status.pendingRestart ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
					className: HarnessUpdateAction_module_css_default.pending,
					role: "status",
					children: t("harnessPendingRestart", { version: status.installedVersion ?? "?" })
				}) : status.updateAvailable && status.latestVersion !== null ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
					className: HarnessUpdateAction_module_css_default.panelActions,
					children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
						className: HarnessUpdateAction_module_css_default.primaryButton,
						type: "button",
						disabled: working,
						onClick: startPlan,
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(__deepseek_ai_dsh_client_ui_primitives.IconDownloadOutline16, { "aria-hidden": "true" }), working ? t("harnessUpdating") : t("harnessUpdate", { version: status.latestVersion })]
					})
				}) : status.updateCheckError !== null ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
					className: HarnessUpdateAction_module_css_default.status,
					children: t("harnessCheckFailed", { message: status.updateCheckError })
				}) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
					className: HarnessUpdateAction_module_css_default.status,
					children: t("harnessUpToDate")
				})] }) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
					className: HarnessUpdateAction_module_css_default.status,
					children: status.installMessage === null ? t("harnessReasonNotManaged") : t("harnessNotManaged", { root: status.installRoot ?? status.executablePath ?? "?" })
				}),
				status !== null ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
					className: HarnessUpdateAction_module_css_default.refreshButton,
					type: "button",
					disabled: working,
					onClick: () => {
						setBanner(null);
