...
var ProfileOperations = class {
	...
	async planStillTargetsCurrentState(plan) {
		if (plan.action === "pause" || plan.action === "resume") {
			...
			if (row$1 === void 0 || !row$1.directDependency || row$1.system || row$1.version !== plan.currentVersion) return false;
		...
		if (plan.action === "remove") {
			const row$1 = (await this.options.profile.list("zh", false)).find((item) => item.packageName === plan.packageName);
			return row$1 !== void 0 && row$1.directDependency && !row$1.system && row$1.version === plan.currentVersion;
		}
	...
	async runPlan(plan, backups) {
		if (plan.action === "pause" || plan.action === "resume") {
			...
			const composition$1 = await this.runCommand([
				"--profile",
				this.options.profile.runtime.profileName,
				"--dump-config"
			], this.options.profile.runtime.dir, this.timeoutMs);
			if (composition$1.unavailable || composition$1.timedOut || composition$1.code !== 0) {
				const rollback = await this.rollbackActivation(backups);
				return snapshotResult(this.options.profile, plan.action, "composition-validation-failed", plan.packageName, false, rollback, composition$1.output);
			}
			if (composition$1.stdoutTruncated || !activationDumpMatches(composition$1.stdout ?? composition$1.output, targets, paused)) {
				const rollback = await this.rollbackActivation(backups);
	...
	async rollback(backups, repairPackages = true) {
		try {
