...
const MAX_PACKAGE_JSON_BYTES$1 = 512e3;
const NPM_TIMEOUT_MS$1 = 8e3;
const NPM_CACHE_MS$1 = 300 * 1e3;
const MAX_NPM_METADATA_BYTES$1 = 128e3;
...
function pathCandidates(dshBin) {
	const candidates = [];
	if (dshBin.includes("/") || dshBin.includes("\\") || isAbsolute(dshBin)) candidates.push(resolve(dshBin));
	else for (const dir of (process.env.PATH ?? "").split(delimiter).filter(Boolean)) {
		candidates.push(join(dir, dshBin));
		if (process.platform === "win32") candidates.push(`${join(dir, dshBin)}.cmd`, `${join(dir, dshBin)}.exe`, `${join(dir, dshBin)}.ps1`);
	}
	return candidates;
}
async function resolveExecutable(dshBin) {
	for (const candidate of pathCandidates(dshBin)) try {
		return await realpath(candidate);
	} catch {}
	return null;
}
async function packageRootFromExecutable(executablePath) {
	let current = dirname(executablePath);
	for (let depth = 0; depth < PACKAGE_ROOT_DEPTH; depth += 1) {
		try {
			const raw = await readTextBounded(join(current, "package.json"), MAX_PACKAGE_JSON_BYTES$1);
			const value = JSON.parse(raw);
			if (isRecord(value) && value.name === HARNESS_PACKAGE && typeof value.version === "string") return {
				root: current,
...
*/
async function npmPrefixForRoot(installRoot) {
	...
	if (basename(layoutParent) === "lib") {
		...
		return await pathExists$1(join(prefix, "bin")) ? prefix : null;
	}
	if (process.platform === "win32") {
		const prefix = layoutParent;
		const child = relative(join(prefix, "node_modules"), installRoot);
		if (child.startsWith("..") || isAbsolute(child)) return null;
		return await pathExists$1(join(prefix, "node_modules", ".bin")) ? prefix : null;
	}
	return null;
}
async function resolveInstallation(dshBin) {
...
function parseVersionField(value) {
	return isRecord(value) && typeof value.version === "string" && semver.valid(value.version) !== null ? value.version : null;
}
/**
...
*/
var HarnessManager = class HarnessManager {
	dshBin;
	fetchImpl;
	now;
	distTagsCache = /* @__PURE__ */ new Map();
	distTagsInflight = null;
	static CACHE_KEY = "dist-tags";
	constructor(options) {
	...
	*/
	distTags() {
		...
		if (cached !== void 0 && cached.expiresAt > this.now()) return Promise.resolve(cached.value);
	...
	async fetchDistTags() {
		...
		try {
			...
			const response = await this.fetchImpl(`https://registry.npmjs.org/-/package/${escaped}/dist-tags`, {
				signal: AbortSignal.timeout(NPM_TIMEOUT_MS$1),
				headers: {
					accept: "application/json",
					"user-agent": "dsh-plugin-console"
				}
			...
			if (!response.ok) value = {
				channels: [],
				error: `npm returned HTTP ${String(response.status)} for the dist-tags document.`
			};
			else {
				const parsed = JSON.parse(await readResponseTextBounded(response, MAX_NPM_METADATA_BYTES$1));
				const channels = isRecord(parsed) ? Object.entries(parsed).filter((entry) => typeof entry[1] === "string" && semver.valid(entry[1]) !== null).map(([tag, version]) => ({
					tag,
					version
				})).sort((left, right) => left.tag.localeCompare(right.tag)) : [];
				value = channels.length === 0 ? {
					channels: [],
					error: "The npm dist-tags document is invalid or has no usable channels."
				} : {
					channels,
					error: null
				};
		...
		this.distTagsCache.set(HarnessManager.CACHE_KEY, {
			expiresAt: this.now() + NPM_CACHE_MS$1,
			value
		});
		return value;
	}
	/** Read the package version currently installed at a package root. */
	async installedVersionAt(installRoot) {
		try {
			const raw = await readTextBounded(join(installRoot, "package.json"), MAX_PACKAGE_JSON_BYTES$1);
			return parseVersionField(JSON.parse(raw));
		} catch {
			return null;
		}
	}
	...
	* immediately.
	*/
	async status(currentVersion, refresh = false) {
	...
	*/
	async cachedStatus(currentVersion) {
		const installation = await this.resolve();
		const entry = this.distTagsCache.get(HarnessManager.CACHE_KEY);
		if (entry === void 0 || entry.expiresAt <= this.now()) this.distTags();
		return this.project(currentVersion, installation, entry?.value ?? {
			channels: [],
			error: null
		});
	}
	project(currentVersion, installation, tags) {
		const installedVersion = installation.status !== "unresolved" ? installation.version : null;
		const candidate = tags.channels.length === 0 ? null : tags.channels.reduce((best, channel) => semver.gt(channel.version, best.version) ? channel : best);
		const updateAvailable = installation.status === "managed" && currentVersion !== null && installedVersion !== null && installedVersion === currentVersion && candidate !== null && semver.gt(candidate.version, installedVersion);
...
async function materializeExternalLinks(sourceRoot, targetRoot) {
	...
	while (pending.length > 0) {
		...
		for await (const entry of directory) {
			...
			if (pathWithin$1(resolve(dirname(targetPath), link), targetRoot)) continue;
			let sourceActual;
			try {
...
async function assertNoExternalLinks(targetRoot) {
	...
	while (pending.length > 0) {
		...
		for await (const entry of directory) {
			...
			if (!stat$1.isSymbolicLink()) continue;
			if (!pathWithin$1(resolve(dirname(path), await readlink(path)), targetRoot)) throw new Error(`Dependency link escapes its private tree: ${path}`);
		}
	}
}
...
async function prepareCanaryWorkspace(profileDir$1, profileName) {
	...
	try {
		...
		for (const filename of [
			...
			"pnpm-workspace.yaml",
			"cordis.patch.yml"
		]) await copyIfPresent(join(profileDir$1, filename), join(canaryProfileDir, filename));
		const homePatch = join(root, "cordis.patch.yml");
		await writeFile(homePatch, "[]\n", {
			encoding: "utf8",
			mode: 384
		});
		const profilesRoot = dirname(profileDir$1);
		...
		await copyCanaryDependencies(profileDir$1, canaryProfileDir);
		return {
			root,
			profileDir: canaryProfileDir,
			cwd
		};
	} catch (error) {
		try {
...
async function prepareCanaryHome(request) {
	const { root, profileDir: profileDir$1, cwd } = await prepareCanaryWorkspace(request.profileDir, request.profileName);
	try {
...
function startProcess(dshBin, profileName, prepared, nonce) {
	let stdout = "";
	let stderr = "";
	let startError = null;
...
export default { name, inject, apply }
`;
async function prepareHarnessCanaryHome(profileDir$1, profileName) {
	const { root, cwd } = await prepareCanaryWorkspace(profileDir$1, profileName);
	try {
		const nonce = randomUUID();
		...
		const probePath = join(root, "probe.mjs");
		const patchPath = join(root, "canary.patch.yml");
		await writeFile(probePath, HARNESS_PROBE_SOURCE, {
			encoding: "utf8",
			mode: 384
		});
		await writeFile(patchPath, stringify([{ insert: [{
			id: probeId,
			name: probePath,
			config: {
				nonce,
...
function parseHarnessProbeDocument(text, nonce, pid) {
	try {
		const value = JSON.parse(text);
		if (!isRecord(value) || value.protocolVersion !== 1 || value.nonce !== nonce || value.pid !== pid || !Array.isArray(value.entries) || !Array.isArray(value.clientGraphEntries) || !(typeof value.port === "number" || value.port === null) || !(value.error === void 0 || typeof value.error === "string")) return null;
		if (value.entries.some((entry) => !isRecord(entry) || typeof entry.id !== "string" || typeof entry.name !== "string" || typeof entry.disabled !== "boolean" || !(typeof entry.state === "number" || entry.state === null) || !Array.isArray(entry.missingServices) || entry.missingServices.some((service) => typeof service !== "string"))) return null;
		if (value.clientGraphEntries.some((entry) => !isRecord(entry) || typeof entry.id !== "string" || !(typeof entry.url === "string" || entry.url === null))) return null;
		if (value.port !== null && (!Number.isSafeInteger(value.port) || value.port <= 0 || value.port > 65535)) return null;
		return value;
	} catch {
		return null;
...
function harnessCompositionFailure(document, expectedEntries) {
	const key = (id, name$1) => `${id}\0${name$1}`;
	const probeCounts = /* @__PURE__ */ new Map();
	...
	for (const expected of expectedEntries) {
		...
		const entry = document.entries.find((candidate) => candidate.id === expected.id && candidate.name === expected.name);
		if (expected.disabled !== "unknown" && entry.disabled !== expected.disabled) return `Loader entry ${expected.id} (${expected.name}) changed activation state (disabled=${String(entry.disabled)}).`;
	...
	if (probeCounts.size > 0) {
		...
		if (failed !== void 0) return `Runtime-only Loader entry ${failed.id} (${failed.name}) failed to activate after the Harness update.`;
	}
	return null;
}
const FIBER_ACTIVE = 2;
async function checkHarnessHttp(port, expectedClientPackages, graphEntries) {
	if (port === null || !Number.isSafeInteger(port) || port <= 0) return {
		ok: false,
		detail: "The isolated Web server did not report a valid port."
	};
	try {
		...
		for (const packageName of expectedClientPackages) {
			...
			if (matches.length === 0) return {
				ok: false,
				detail: `Client package ${packageName} is missing from the Web module graph after the Harness update.`
			};
			if (matches.length > 1) return {
				ok: false,
				detail: `Client package ${packageName} has multiple Web module graph rows after the Harness update.`
			};
			const url = matches[0]?.url;
			if (!url.startsWith("/plugins/") || !url.includes("/client.js")) return {
				ok: false,
				detail: `Client package ${packageName} serves an unexpected bundle URL (${url}).`
			};
			...
			if (!bundleResponse.ok) {
				...
				return {
					ok: false,
					detail: `Client bundle ${packageName} returned HTTP ${String(bundleResponse.status)}.`
				};
			}
			const validated = validateClientBundle(await readResponseTextBounded(bundleResponse, CLIENT_SCRIPT_MAX_BYTES), packageName, graphIds);
		...
		return {
			ok: false,
			detail: "The Web root or a client bundle could not be fetched."
		};
	}
}
async function runHarnessUpdateCanary(request) {
	if (request.expectedEntries.length === 0) return {
		status: "failed",
		code: "harness-canary-composition-failed",
		detail: "The profile composition has no Loader entries to verify."
	};
	let prepared;
	try {
	...
	let outcome = {
		status: "failed",
		code: "harness-canary-start-failed",
		detail: null
	};
	try {
		try {
		...
		if (running !== null) {
			...
			if (document === null && outcome.code === "harness-canary-start-failed" && outcome.detail === null) outcome = {
				status: "failed",
				code: "harness-canary-timeout",
				detail: running.output() ?? "The isolated DSH startup did not settle before the deadline."
			};
			else if (document !== null) if (document.error !== void 0) outcome = {
				status: "failed",
				code: "harness-canary-composition-failed",
				...
				if (composition !== null) outcome = {
					...
					if (running.child.exitCode !== null || running.child.signalCode !== null) outcome = {
						status: "failed",
						code: "harness-canary-process-exited",
						detail: running.output()
					};
					else {
						...
						outcome = http.ok ? {
							status: "passed",
							code: "harness-canary-passed",
							detail: null
						} : {
							status: "failed",
							code: "harness-canary-http-failed",
							detail: http.detail
						};
					}
				}
		...
		if (!shutdown) outcome = {
			status: "failed",
			code: "harness-canary-shutdown-failed",
			detail: "The isolated DSH process could not be terminated cleanly."
		};
		else if (!cleanup) outcome = {
			status: "failed",
			code: "harness-canary-cleanup-failed",
			detail: "The isolated DSH process stopped, but its temporary profile could not be removed."
		};
	}
	return outcome;
...
const CAPABILITIES_CACHE_MS = 3e4;
...
var ProfileManager = class {
	...
	*/
	async capabilities() {
		...
		if (cached !== null && cached.expiresAt > Date.now()) return {
			...cached.value,
			busy: this.busy
		};
...
var ProfileOperations = class {
	...
	storePlan(input) {
		...
		const expiresAt = new Date(this.now() + PLAN_TTL_MS$1).toISOString();
		const plan = {
			status: "ready",
			planId,
			blockReason: null,
...
//#endregion
//#region src/harness-operations.ts
...
*/
function evaluateJsExpression(expression) {
	try {
		return new Function("process", `"use strict"; return (${expression});`)(process);
	} catch {
		return { __jsExpr: expression };
	}
}
const EVALUATED_JS_TAG = {
	tag: "tag:yaml.org,2002:js",
	identify: (value) => isRecord(value) && Object.keys(value).length === 1 && typeof value.__jsExpr === "string",
	resolve: (value) => evaluateJsExpression(value),
	stringify: ({ value }) => isRecord(value) && typeof value.__jsExpr === "string" ? value.__jsExpr : ""
};
function composedEntriesFromDump(output) {
	if (output === null || output === void 0) return null;
	try {
		const entries = [];
		collectComposedEntries(parse(output, { customTags: [EVALUATED_JS_TAG] }), entries);
		const mapped = entries.filter((entry) => typeof entry.id === "string" && typeof entry.name === "string").map((entry) => ({
			id: entry.id,
			name: entry.name,
			disabled: typeof entry.disabled === "boolean" ? entry.disabled : "unknown"
		}));
...
function blockedPlan(profileName, reason, harness) {
	return {
		status: "blocked",
		planId: null,
		blockReason: reason,
...
async function verifyInstalledVersion(harness, runCommand, profileDir$1, timeoutMs, expectedVersion, previousInstallRoot) {
	const installation = await harness.resolve();
	if (installation.status !== "managed" || installation.version !== expectedVersion || previousInstallRoot !== null && installation.installRoot !== previousInstallRoot) return {
		ok: false,
		installation,
		detail: `Expected the npm-global installation to be ${expectedVersion}, found ${installation.version ?? "unknown"}.`
	};
	if (installation.executablePath === null) return {
		ok: false,
		installation,
		detail: "The updated dsh executable could not be resolved."
	};
	let version;
	try {
		version = await runCommand(installation.executablePath, ["--version"], profileDir$1, Math.min(timeoutMs, 3e4));
	} catch (error) {
		return {
			ok: false,
			installation,
	...
	if (version.unavailable || version.timedOut || version.code !== 0 || (version.stdout ?? version.output ?? "").includes(expectedVersion) === false) return {
		ok: false,
		installation,
...
var HarnessOperations = class {
	...
	async snapshot(status, code, plan, restartRequired, rollback, detail, canary = "not-run", processCleanup = "not-needed") {
		...
		return {
			...
			restartRequired,
			activation: status === "succeeded" && restartRequired ? "pending-restart" : rollback === "failed" || processCleanup === "failed" ? "unknown" : "unchanged",
			canary,
			processCleanup,
			rollback,
			detail: detail ?? snapshotError,
	...
	async blockedFromStatus(reason) {
		const status = await this.options.harness.status(this.options.runningVersion);
	...
	async execute(planId) {
		...
		const stored = this.plans.get(planId);
		this.plans.delete(planId);
		if (stored === void 0) return this.snapshot("failed", "plan-invalid-or-expired", null, false, "not-needed", null);
		...
		this.options.profile.setBusy(true);
		const operation = this.prepareAndRun(stored).then(async (result) => ({
			...result,
			capabilities: {
				...result.capabilities,
				busy: false
	...
	async prepareAndRun(stored) {
		...
		try {
			if (stored.profileFingerprint !== this.options.profile.fingerprint()) return this.snapshot("failed", "profile-changed", plan, false, "not-needed", null);
			const status = await this.options.harness.status(this.options.runningVersion);
			if (!status.managed || status.installedVersion !== plan.currentVersion || status.executablePath !== stored.executablePath) return this.snapshot("failed", "harness-state-changed", plan, false, "not-needed", null);
			if (status.latestVersion === null || status.latestVersion !== plan.targetVersion || !status.updateAvailable) return this.snapshot("failed", "harness-state-changed", plan, false, "not-needed", null);
			const profileDir$1 = this.options.profile.runtime.dir;
			const profileName = this.options.profile.runtime.profileName;
			let composition;
			try {
				composition = await this.runCommand(stored.executablePath, [
					"--profile",
					profileName,
					"--dump-config"
				], profileDir$1, this.timeoutMs);
			} catch (error) {
				return this.snapshot("failed", "composition-snapshot-failed", plan, false, "not-needed", errorMessage(error));
			}
			if (composition.unavailable || composition.timedOut || composition.code !== 0 || composition.stdoutTruncated) return this.snapshot("failed", "composition-snapshot-failed", plan, false, "not-needed", composition.output);
			const expectedEntries = composedEntriesFromDump(composition.stdout ?? composition.output);
			if (expectedEntries === null) return this.snapshot("failed", "composition-snapshot-failed", plan, false, "not-needed", "The composed profile dump has no Loader entries.");
			const expectedClientPackages = (await this.options.profile.list("zh", false)).filter((row) => row.client && row.activeAfterRestart).map((row) => row.packageName);
			const args = [
				"install",
				"--global",
				"--prefix",
				plan.prefix,
				`@deepseek-ai/dsh@${plan.targetVersion}`
			];
			let update;
			try {
				update = await this.runCommand(this.options.npmBin, args, profileDir$1, this.timeoutMs);
			} catch (error) {
				return this.rollbackFrom(plan, expectedEntries, expectedClientPackages, null, profileDir$1, profileName, errorMessage(error), true);
			}
			if (update.unavailable) return this.rollbackFrom(plan, expectedEntries, expectedClientPackages, update, profileDir$1, profileName, update.output, true);
			if (update.timedOut) {
				if (update.processCleanup === false) return this.snapshot("failed", "operation-process-leaked", plan, false, "failed", update.output, "not-run", "failed");
				return this.rollbackFrom(plan, expectedEntries, expectedClientPackages, update, profileDir$1, profileName, update.output, true, "succeeded");
			}
			if (update.code !== 0) return this.rollbackFrom(plan, expectedEntries, expectedClientPackages, update, profileDir$1, profileName, update.output, true);
			const verified = await verifyInstalledVersion(this.options.harness, this.runCommand, profileDir$1, this.timeoutMs, plan.targetVersion, plan.installRoot);
			if (!verified.ok) return this.rollbackFrom(plan, expectedEntries, expectedClientPackages, null, profileDir$1, profileName, verified.detail, true);
			let canary;
			try {
			...
			if (canary.status !== "passed") return this.rollbackFrom(plan, expectedEntries, expectedClientPackages, null, profileDir$1, profileName, canary.detail, true, "succeeded");
			return this.snapshot("succeeded", "succeeded", plan, true, "not-needed", null, "passed", "succeeded");
		} finally {
			await release().catch(() => void 0);
		}
	}
	async rollbackFrom(plan, expectedEntries, expectedClientPackages, failedUpdate, profileDir$1, profileName, detail, mutationSuspected, failedProcessCleanup = failedUpdate?.timedOut === true ? failedUpdate.processCleanup === false ? "failed" : "succeeded" : "not-needed") {
		if (!mutationSuspected) return this.snapshot("failed", "harness-update-command-failed", plan, false, "not-needed", detail, "not-run", failedProcessCleanup);
		const current = await this.options.harness.resolve();
		if (current.status === "managed" && current.version === plan.currentVersion) return this.snapshot("failed", "harness-update-command-failed", plan, false, "not-needed", detail, "not-run", failedProcessCleanup);
		try {
			const args = [
				"install",
				"--global",
				"--prefix",
				plan.prefix,
				`@deepseek-ai/dsh@${plan.currentVersion}`
			];
			const restore = await this.runCommand(this.options.npmBin, args, profileDir$1, this.timeoutMs);
			if (restore.unavailable || restore.timedOut || restore.code !== 0) return this.snapshot("failed", "harness-update-command-failed", plan, false, "failed", `${detail ?? ""} ${restore.output ?? "Rollback reinstall failed."}`.trim(), "not-run", failedProcessCleanup);
			const restored = await verifyInstalledVersion(this.options.harness, this.runCommand, profileDir$1, this.timeoutMs, plan.currentVersion, plan.installRoot);
			if (!restored.ok) return this.snapshot("failed", "harness-update-command-failed", plan, false, "failed", `${detail ?? ""} ${restored.detail ?? "Rollback verification failed."}`.trim(), "not-run", failedProcessCleanup);
			let canary;
			try {
				canary = await this.probeActivation({
					profileDir: profileDir$1,
			...
			if (canary.status !== "passed") return this.snapshot("failed", "harness-update-command-failed", plan, false, "failed", `${detail ?? ""} ${canary.detail ?? "Rollback canary failed."}`.trim(), "failed", failedProcessCleanup);
...
const BOOTSTRAP_SEED_WAIT_MS = 3500;
...
async function apply(ctx, config) {
	...
	if (config.npmBin.trim().length === 0 || config.npmBin.includes("\0")) throw new Error("dsh-plugin-console npmBin is invalid.");
	...
	const runningVersion = (await harness.resolve()).version;
			...
			try {
				...
				switch (method) {
					case "bootstrap": {
						...
						const bootstrapLocale = locale(isRecord(params) ? params.locale : void 0);
						const status = catalog.status();
						if (status.state === "unavailable") await Promise.race([catalog.refresh(), new Promise((resolve$1) => {
							setTimeout(resolve$1, BOOTSTRAP_SEED_WAIT_MS).unref();
						})]);
						else if (status.stale) catalog.refresh();
...
//#endregion
export { Config, HarnessManager, HarnessOperations, PluginCatalog, ProfileManager, ProfileOperations, apply, composedEntriesFromDump, src_default as default, inject, name, parseCatalogText, queryCatalog, runActivationCanary, runHarnessUpdateCanary };
